<template>
  <v-row :id="id" v-bind="getRow" v-resize.quiet="onResize">
    <!-- FORM-BASE TOP SLOT -->
    <slot :id="id" :name="getFormTopSlot()" />
    ...
    <slot :id="id" :name="getFormBottomSlot()" />
  </v-row>
</template>
...
export default {
  ...
  methods: {
    ...
    drop(event, obj) {
      ...
      if (isFunction(obj.schema.drop)) {
        obj.value = this.dropValue(obj, event);
      }
    ...
    onClickOutside(event, obj) {
      if (!obj.schema || !obj.schema.clickOutside) {
        return;
      }
    ...
    deprecateEventCustomID(ev) {
      ...
      console.warn(
        `<v-form-base  @${ev}:${this.id}="handler" /> is deprecated use simplified version <v-form-base  @${ev}="handler" />`
      );
      console.warn(
        "---------------------------------------------------------------------------------------------"
      );
    ...
    deprecateCombinedEvents(emitEvent, event) {
      if (emitEvent !== event) {
        ...
        console.warn(
          'Please use separate listener for each event like <v-form-base  @focus="handler" @input="handler" @blur="handler"/>'
        );
        console.warn(
          "---------------------------------------------------------------------------------------------"
        );
    ...
    flattenObjects(dat = {}, sch) {
      ...
      Object.keys(sch).forEach((key) => {
        ...
        if (
          ...
          const { data: flatData, schema: flatSchema } = this.flattenObjects(
            dat[key],
            sch[key]
          );
    ...
    autogenerateSchema(value) {
      ...
      schema = JSON.parse(schema, (key, val) => {
        ...
        if (typeof val === "string") {
          return defaultSchemaIfValueIsString(key);
        }
        if (typeof val === "number") {
          return defaultSchemaIfValueIsNumber(key);
        }
